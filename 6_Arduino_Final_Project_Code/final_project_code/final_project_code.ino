...
#define MODE_3 3  // Send peak values
#define MODE_4 4  // Send peak times
#define MODE_5 5  // Send peak periods

// Circular queue buffer size
...
const int numReadings = 20;  // For averaging/smoothing
...
unsigned long cqBuffer[BUFFER_SIZE];
unsigned int cQueueStart = 0;
unsigned int cQueueEnd = 0;
unsigned int cQueueSize = 0;

// Peak buffer
unsigned long peakTimesArray[PEAK_ARRAY_SIZE];  // Array that stores all the peak times
unsigned long peakValuesArray[PEAK_ARRAY_SIZE]; // Array that stores all the peak values
unsigned long peakIndex = 0;  // the index of the peak times and value array

// Array that stores the previous average values
unsigned long prevAverageArray[PREV_ARRAY_SIZE];

unsigned long lastPeakSmall = FALSE;
...
void ResetPeakArrays(void) {
  // Reset all elements in the peak times array to 0
  for (int index = 0; index < PEAK_ARRAY_SIZE; index++) {
    peakTimesArray[index] = 0;
    peakValuesArray[index] = 0;
  }
  peakIndex = 0;              // Set the index of the peak to 0
}
...
void GetPeaks(void) {
  // If the middle value is larger than the previous and next value
  // Then we have a peak
  if(prevAverageArray[PREV_ARRAY_SIZE/2] > prevAverageArray[0] && 
    ...
    if(peakIndex == PEAK_ARRAY_SIZE) {  // We have stored 20 peaks
      ...
      peakTimesArray[peakIndex] = millis();
      peakValuesArray[peakIndex++] = prevAverageArray[PREV_ARRAY_SIZE/2];
      lastPeakSmall = FALSE;

      if(arduinoTestFlag == true) {
        Serial.print("Peak: ");
        Serial.print( prevAverageArray[PREV_ARRAY_SIZE/2]);
        Serial.println();
        Serial.print("Peak Time: ");
        Serial.print(peakTimesArray[peakIndex-1]);
        Serial.println();
...
//    dampedPeriod = 0;
//    unsigned long originalQueueSize = cQueueSize;
//    while(cQueueSize>0) {
//      dampedPeriod = dampedPeriod + cDequeue();
//    }
//    GotoXY(0,2);
//    avgDampedPeriod = dampedPeriod/originalQueueSize;
//    LcdWriteString("t_d: ");
//    LcdWriteDec(avgDampedPeriod);
//    LcdWriteString("ms   ");
//
//    GotoXY(0,3);
//    avgDampedFrequency = 1.0/(avgDampedPeriod/1000.0);
//    LcdWriteString("w_d: ");
//    LcdWriteDec(avgDampedFrequency);
//    LcdWriteString("rad/s   ");
//    
//    buttonPressedFlag = 0;
//  }
...
// DEBUGGING COMMANDS
void OutputPeaksToSerial(void) {
  if(arduinoTestFlag==true) {
    if(peakIndex > 0 ) {  // If there is stuff in the array
      Serial.println("Peaks Values: ");
      for(int index = 0; index < peakIndex; index++) {
        Serial.print(peakValuesArray[index]);
        Serial.print(" ");
      }
      Serial.println("Peaks Times: ");
      for(int index = 0; index < peakIndex; index++) {
        Serial.print(peakTimesArray[index]);
        Serial.print(" ");
      }
      Serial.println("Peaks Periods: ");
      for(int index = 0; index < peakIndex-1; index++) {
        Serial.print(peakTimesArray[index+1]-peakTimesArray[index]);
        Serial.print(" ");
      }
    } 
  }else {
    for(int index = 0; index < peakIndex; index++) {
      WriteDecimalToSerial(peakValuesArray[index],MODE_3);
    }
    for(int index = 0; index < peakIndex-1; index++) {
      WriteDecimalToSerial(peakTimesArray[index+1]-peakTimesArray[index],MODE_4);
    }
  }
}
